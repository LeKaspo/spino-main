...
# Start thread to receive serial port data
...
# TCP command timeout count (not received)
g_tcp_except_count = 0

g_motor_speed = [0, 0, 0, 0]

# Return microcontroller version number, tcp=TCP service object
...
# Return robot arm middle offset state
...
# Return Ackermann car default angle
...
def parse_data(sk_client, data):
    ...
    # Set color lamp single-color breathing effect color
    elif cmd == "32":
    ...
    # Read Ackermann car default angle
    elif cmd == '50':
        num_id = int(data[7:9], 16)
        if g_debug:
    ...
    # Temporarily modify Ackermann servo default angle
    elif cmd == '51':
        num_id = int(data[7:9], 16)
        num_angle = int(data[9:11], 16)
        if g_debug:
    ...
    # Confirm Ackermann car default angle modification
    elif cmd == '52':
        num_verify = int(data[7:9], 16)
        if g_debug:
    ...
    # Control Ackermann car front wheel servo angle
    elif cmd == '53':
        num_id = int(data[7:9], 16)
        num_angle = int(data[9:11], 16)
        if num_angle > 127:
...
# Run program according to state machine including video stream return
def mode_handle():
...
if __name__ == '__main__':
    ...
    # Eigene Programme
    print(g_bot.get_battery_voltage())
